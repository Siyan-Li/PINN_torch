...
# 定义初始条件和边界条件损失
def icbc_loss(model, x_ic, t_ic, u_ic, x_bc1, t_bc1, x_bc2, t_bc2, q0, q1):
    # 初始条件损失
...
def generate_data():
    ...
    # 诺依曼边界条件，假设边界热流密度为0（绝热）
    q0 = torch.zeros_like(t_bc)
    q1 = torch.zeros_like(t_bc)
    # 内部数据
    x = torch.rand(1000, 1) * (x_max - x_min) + x_min
    t = torch.rand(1000, 1) * (t_max - t_min) + t_min

    return x, t, x_ic, t_ic, u_ic, x_bc1, t_bc1, x_bc2, t_bc2, q0, q1
...
if __name__ == "__main__":
    ...
    plt.title('PINN solution of the heat equation with Neumann BC')
    plt.show()
