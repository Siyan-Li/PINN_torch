...
# 定义初始条件和边界条件损失
def icbc_loss(model, x_ic, t_ic, u_ic, x_bc1, t_bc1, x_bc2, t_bc2, u_bc1_value, u_bc2_value):
    # 初始条件损失
...
def generate_data():
    ...
    # 狄利克雷边界条件，边界温度
    u_bc1_value = torch.full_like(t_bc, 0.2)
    u_bc2_value = torch.full_like(t_bc, 0.8)
    # 内部数据
    x = torch.rand(1000, 1) * (x_max - x_min) + x_min
    t = torch.rand(1000, 1) * (t_max - t_min) + t_min

    return x, t, x_ic, t_ic, u_ic, x_bc1, t_bc1, x_bc2, t_bc2, u_bc1_value, u_bc2_value


# 训练模型
def train(model, alpha, epochs=1000, lr_model=0.005, lr_alpha=0.00001):
    alpha = torch.tensor(alpha, requires_grad=True)
    optimizer = torch.optim.Adam([
        {'params': model.parameters(), 'lr': lr_model},
        {'params': [alpha], 'lr': lr_alpha}
    ])
    x, t, x_ic, t_ic, u_ic, x_bc1, t_bc1, x_bc2, t_bc2, q0, q1 = generate_data()

    for epoch in range(epochs):
        optimizer.zero_grad()
        loss_pde = pde_loss(model, x, t, alpha)
        loss_icbc = icbc_loss(model, x_ic, t_ic, u_ic, x_bc1, t_bc1, x_bc2, t_bc2, q0, q1)
